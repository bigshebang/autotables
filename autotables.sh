...
while [ true ]; do #main while loop
	...
	if [ "$option" == "1" ]; then #drop option
		...
		if [ "$restoreFile" == "" ]; then
			if [ "$lastBackup" == "" ]; then
				...
				for file in $files; do
					...
					if [ "${head:0:28}" == "# Generated by iptables-save" ]; then
						echo "File found!"
						restoreFile="$file"
						lastBackup="$file"
						break
		...
		if [ "$restoreFile" != "" -a -e "$restoreFile" ]; then
		...
		while [ true ]; do
			...
			$path/iptables -F
			if [ "$answer" == "1" ]; then #bare bones option
				#logs
				...
				break
			elif [ "$answer" == "2" ]; then # moderate option
				#logs
				...
				break
			elif [ "$answer" == "3" ]; then #high option
				#logs
				...
				break
			elif [ "$answer" == "4" ]; then #impenetrable option
				#log everything
